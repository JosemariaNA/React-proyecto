# 🌐 Portafolio Personal - React

Este es mi proyecto de **Portafolio Personal**, desarrollado con **React.js**.  
En él presento mis habilidades, proyectos y experiencia como estudiante de **Ingeniería de Sistemas**.

---

## 🚀 Tecnologías utilizadas

- React.js  
- HTML5, CSS3 y JavaScript  
- Node.js  
- Git y GitHub  
- Vite / Create React App  

---

## ⚙️ Instalación y ejecución

Sigue estos pasos para ejecutar el proyecto en tu entorno local:

1. **Clona el repositorio**
   ```bash
   git clone https://github.com/JosemariaNA/React-proyecto.git
   ```

2. **Accede al directorio del proyecto**
   ```bash
   cd mi-portafolio
   ```

3. **Instala las dependencias**
   ```bash
   npm install
   ```

4. **Inicia el servidor de desarrollo**
   ```bash
   npm start
   ```

5. Abre el proyecto en tu navegador en:  
   👉 [http://localhost:5173](http://localhost:5173)

---

## 🧠 Descripción del proyecto

El portafolio muestra información personal, proyectos realizados, contacto y detalles académicos.  
Incluye un diseño moderno, adaptable (responsive) y animaciones suaves para una mejor experiencia visual.

---

## 📸 Capturas de pantalla



```markdown
![Vista principal](https://github.com/JosemariaNA/React-proyecto/blob/main/react/src/assets/capturas/captura1.png?raw=true)
![Seccion Sobre Mi](https://github.com/JosemariaNA/React-proyecto/blob/main/react/src/assets/capturas/captura2.png?raw=true)
```

---

## 🧑‍💻 Autor

**Josemaria Napan Aparcana**  
📧 [napanjosemaria@gmail.com](mailto:napnjosemaria@gmail.com)  
🌍 [LinkedIn](https://www.linkedin.com/in/josemaría-napan-aparcana-a446a7332) | [GitHub](https://github.com/JosemariaNA)

---

## 📜 Licencia

Este proyecto está bajo la licencia **MIT**, por lo que puedes usarlo, modificarlo y distribuirlo libremente citando al autor original.
